...
<div class="container col-md-7">
	<form method="POST" action="<?= site_url('p/act_driver') ?>" enctype="multipart/form-data">
	<div class="row" style="margin-top: -100px;">
			<div class="card w-100" style="border-radius: 0px;background-color: #212529;">
				<div class="card-body text-center">
					<div class="text-left text-light"><h2 style="font-family: Montserrat;"><b style="background: -webkit-linear-gradient(#00d2ff,#3a7bd5);-webkit-background-clip: text;-webkit-text-fill-color: transparent;">Form</b> Driver</h2></div>
				</div>
			</div>
	...
	<div class="row">
		...
		<div class="card w-100" style="border-top-left-radius: 0px;border-top-right-radius: 0px;">
			<div class="card-body">
				...
				<div class="input-group mb-3">
					<div class="form-floating w-100">
					  <input type="date" class="form-control input-sm" id="birth_date" placeholder="Tanggal Lahir" name="tanggal_lahir" required>
	...
	<div id="field_docMultiple">
		<div class="row mt-3">
			<div class="card w-100">
				<div class="card-body">
					<div class="card-title">SIM</div><hr>
					<div class="mb-3">
						<select class="form-select mb-3" data-sim="simSelector1" onchange="openDocField(this)" required="">
							<option hidden="" disabled="disabled" selected="selected" value="">Pilih</option>
							<option value="a">SIM A</option>
							<option value="b1">SIM B1</option>
							<option value="b2">SIM B2</option>
						</select>
  						<input class="form-control" type="file" id="simSelector1" onchange='ValidateSingleInput(this)' name="" required disabled>
  						<p class="text-center text-secondary" style="font-size: 15px;margin-top: 20px;">Wajib Mengupload Minimal 1 Foto SIM (Format PNG/JPG)</p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="card w-100">
			<div class="card-body">
				<div class="d-grid gap-2">
					<input type="button" id="btn_addDoc" class="btn btn-secondary" value="Tambah SIM">
...
<script type="text/javascript">
	function openDocField(input){
		var value = input.value;
		var selector = input.dataset.sim;
		document.getElementById(selector).removeAttribute("disabled");	
		document.getElementById(selector).setAttribute("name",value);	
	}
		        ...
		        var sFileName = oInput.value;
		         if (sFileName.length > 0) {
		            ...
		            if (!blnValid) {
		                Swal.fire(
						  'Format File Salah',
						  'Format yang diizinkan: '+_validFileExtensions.join(", "),
						  'error'
						)
		                oInput.value = "";
		                return false;
	...
	$(document).ready(function(){
		...
		$('#btn_addDoc').on('click', function(){
			...
			var sim_selector = 'simSelector'+total_doc;
			$('#field_docMultiple').append("<div class='row mt-3' id='docField"+total_doc+"'><div class='card w-100'><div class='card-body'><i class='fa fa-times text-secondary' style='float:right;cursor: pointer;' onclick='remove(docField"+total_doc+")'></i><div class='card-title'>SIM</div><hr><div class='mb-3'><select class='form-select mb-3' data-sim='"+sim_selector+"' onchange='openDocField(this)' required=''><option hidden='' disabled='disabled' selected='selected' value=''>Pilih</option><option value='a'>SIM A</option><option value='b1'>SIM B1</option><option value='b2'>SIM B2</option></select><input class='form-control' type='file' id='"+sim_selector+"' onchange='ValidateSingleInput(this)' name='' required disabled></div></div></div></div>");
		});
