<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Member Driver</h1>
...
function openDoc(n){
    var a 	= n.getAttribute('data-sima');
    var b1 	= n.getAttribute('data-simb1');
    var b2 	= n.getAttribute('data-simb2');

    swal.fire({
        title: 'Data SIM Member',
        icon: 'question',
        html: "Pilih salah satu yang ingin diperiksa" +
            "<br><br>" +
            '<button type="button" role="button" tabindex="0" onclick="openSim(`'+a+'`)" class="btn btn-primary">' + 'SIM A' + '</button> ' +
            '<button type="button" role="button" tabindex="0" onclick="openSim(`'+b1+'`)" class="btn btn-info">' + 'SIM B1' + '</button> ' +
            '<button type="button" role="button" tabindex="0" onclick="openSim(`'+b2+'`)" class="btn btn-warning">' + 'SIM B2' + '</button>',
...
function openSim(n){
	if(n != '-'){
	Swal.fire({
	  imageUrl: '<?=base_url()?>assets/uploads/'+n,
	  imageHeight: 400,
	  imageAlt: 'SIM image',
      ...
      showConfirmButton: false
		...
		Swal.fire(
		  'Tidak Ditemukan',
		  'Terdaftar tidak mengupload SIM ini',
		  'warning'
		)
	}
}
$(document).ready(function(){
    ...
    function loadTable(pagno){
       ...
       $.ajax({
         url: '<?=base_url()?>AdminArea/jsonDRV/'+pagno,
         type: 'get',
    ...
    function setData(result,sno){
       ...
       for(index in result){
          ...
          var sim_a  				= result[index].sim_a;
          var sim_b1  				= result[index].sim_b1;
          var sim_b2  				= result[index].sim_b2;
          ...
          sno++;
            ...
            var tr = "<div class='row p-1 mb-2'>"
                ...
                tr += "<div class='col text-right'><button class='btn btn-primary btn-sm p-0 pl-1 pr-1' data-toggle='modal' data-target='#detail"+id_member+"'>Detail</button> <button class='btn btn-danger btn-sm p-0 pl-1 pr-1' data-sima = '"+sim_a+"' data-simb1 = '"+sim_b1+"' data-simb2 = '"+sim_b2+"' onclick='openDoc(this)'>Surat Izin Mengemudi</button></div>"
                tr += "</div>"
